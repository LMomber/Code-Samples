...
{
public:
    MakeUploadBuffer(Device& device, size_t pageSize = _2MB) : UploadBuffer(device, pageSize) {}
    ...
    : m_Device(device), m_d3d12CommandListType(type), m_RootSignature(nullptr), m_PipelineState(nullptr)
{
    ...
    ThrowIfFailed(d3d12Device->CreateCommandList(0,
...
void CommandList::TransitionBarrier(Microsoft::WRL::ComPtr<ID3D12Resource> resource,
...
void CommandList::TransitionBarrier(const std::shared_ptr<DX12Resource>& resource,
                                    D3D12_RESOURCE_STATES stateAfter,
                                    UINT subresource,
                                    bool flushBarriers)
{
...
void CommandList::UAVBarrier(const std::shared_ptr<DX12Resource>& resource, bool flushBarriers)
{
...
void CommandList::AliasingBarrier(const std::shared_ptr<DX12Resource>& beforeResource,
                                  const std::shared_ptr<DX12Resource>& afterResource,
                                  bool flushBarriers)
{
...
void CommandList::FlushResourceBarriers() { m_ResourceStateTracker->FlushResourceBarriers(shared_from_this()); }
...
void CommandList::CopyResource(const std::shared_ptr<DX12Resource>& dstRes, const std::shared_ptr<DX12Resource>& srcRes)
{
...
void CommandList::ResolveSubresource(const std::shared_ptr<DX12Resource>& dstRes,
                                     const std::shared_ptr<DX12Resource>& srcRes,
                                     ...
                                     uint32_t srcSubresource)
{
    ...
    m_d3d12CommandList->ResolveSubresource(dstRes->GetD3D12Resource().Get(),
                                           dstSubresource,
...
std::shared_ptr<VertexBuffer> CommandList::CopyVertexBuffer(size_t numVertices,
                                                            size_t vertexStride,
                                                            const void* vertexBufferData)
{
    auto d3d12Resource = CopyBuffer(numVertices * vertexStride, vertexBufferData);
    std::shared_ptr<VertexBuffer> vertexBuffer = m_Device.CreateVertexBuffer(d3d12Resource, numVertices, vertexStride);

    return vertexBuffer;
}

std::shared_ptr<IndexBuffer> CommandList::CopyIndexBuffer(size_t numIndices,
...
std::shared_ptr<StructuredBuffer> CommandList::CopyStructuredBuffer(size_t numElements,
                                                                    size_t elementSize,
...
std::shared_ptr<DX12Texture> CommandList::LoadTextureFromFile(const std::wstring& fileName, bool sRGB)
{
    ...
    else
    {
        ...
        {
            std::shared_ptr<DX12Texture> nullTexture = nullptr;
...
void CommandList::GenerateMips(const std::shared_ptr<DX12Texture>& texture)
{
    if (!texture) return;
    ...
    if (!texture->CheckUAVSupport() || (resourceDesc.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS) == 0)
    {
        ...
        uavDesc.Format = DX12Texture::GetUAVCompatableFormat(resourceDesc.Format);
    ...
    GenerateHzbMips(uavTexture, DX12Texture::IsSRGBFormat(resourceDesc.Format));
    //GenerateMips_UAV(uavTexture, DX12Texture::IsSRGBFormat(resourceDesc.Format));
...
void bee::CommandList::GenerateHzbMips(const std::shared_ptr<DX12Texture>& texture, bool isSRGB)
{
    if (!m_GenerateHzbMipsPSO)
    {
        m_GenerateHzbMipsPSO = std::make_unique<GenerateHzbMipsPSO>(m_Device);
    }

    SetPipelineState(m_GenerateHzbMipsPSO->GetPipelineState());
    SetComputeRootSignature(m_GenerateHzbMipsPSO->GetRootSignature());

    GenerateHzbMipsCB generateMipsCB;
    ...
    srvDesc.Format = isSRGB ? DX12Texture::GetSRGBFormat(resourceDesc.Format) : resourceDesc.Format;
    ...
    for (uint32_t srcMip = 0; srcMip < resourceDesc.MipLevels - 1u;)
    {
        ...
        uavDesc.Texture2D.MipSlice = srcMip + 1;

        auto uav = m_Device.CreateUnorderedAccessView(texture, nullptr, &uavDesc);
        SetUnorderedAccessView(GenerateMips::OutMip, 0, uav, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, srcMip + 1, 1);

        Dispatch(Math::DivideByMultiple(dstWidth, 16), Math::DivideByMultiple(dstHeight, 16));  // CHANGE
...
void CommandList::GenerateMips_UAV(const std::shared_ptr<DX12Texture>& texture, bool isSRGB)
{
    ...
    for (uint32_t srcMip = 0; srcMip < resourceDesc.MipLevels - 1u;)
    {
        ...
        if (mipCount < 4)
        {
            m_DynamicDescriptorHeap[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV]
                ->StageDescriptors(GenerateMips::OutMip, mipCount, 4 - mipCount, m_GenerateMipsPSO->GetDefaultUAV());
        }
...
void CommandList::ClearTexture(const std::shared_ptr<DX12Texture>& texture, const float clearColor[4])
{
    assert(texture);
...
void CommandList::ClearDepthStencilTexture(const std::shared_ptr<DX12Texture>& texture,
...
void CommandList::CopyTextureSubresource(const std::shared_ptr<DX12Texture>& texture,
...
void CommandList::SetGraphicsDynamicStructuredBuffer(uint32_t slot,
                                                     size_t numElements,
                                                     size_t elementSize,
                                                     const void* bufferData)
{
...
void CommandList::SetViewport(const D3D12_VIEWPORT& viewport) { SetViewports({viewport}); }
...
void CommandList::SetScissorRect(const D3D12_RECT& scissorRect) { SetScissorRects({scissorRect}); }
...
void CommandList::SetShaderResourceView(int32_t rootParameterIndex,
                                        uint32_t descriptorOffset,
                                         ...
                                         UINT numSubresources)
{
    if (texture)
    {
        ...
        m_DynamicDescriptorHeap[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV]->StageDescriptors(rootParameterIndex,
                                                                                          descriptorOffset,
                                                                                          1,
                                                                                          texture->GetUnorderedAccessView(mip));
    }
}
...
void CommandList::TrackResource(Microsoft::WRL::ComPtr<ID3D12Object> object) { m_TrackedObjects.push_back(object); }

void CommandList::TrackResource(const std::shared_ptr<DX12Resource>& res)
{
    assert(res);
...
void CommandList::ReleaseTrackedObjects() { m_TrackedObjects.clear(); }
