      ...
      m_Fullscreen(false)
{
    ...
    ThrowIfFailed(
        dxgiFactory5
            ->CreateSwapChainForHwnd(d3d12CommandQueue.Get(), m_hWnd, &swapChainDesc, nullptr, nullptr, &dxgiSwapChain1));
...
{
    ::WaitForSingleObjectEx(m_hFrameLatencyWaitableObject,
                                           1000,
...
void SwapChain::Resize(uint32_t width, uint32_t height)
{
    if (m_Width != width || m_Height != height)
    {
        ...
        ThrowIfFailed(m_dxgiSwapChain->ResizeBuffers(BufferCount,
                                                     m_Width,
                                                     m_Height,
...
UINT SwapChain::Present(const std::shared_ptr<DX12Texture>& texture)
{
