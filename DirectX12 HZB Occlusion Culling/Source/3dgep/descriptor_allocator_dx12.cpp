...
struct MakeAllocatorPage : public bee::DescriptorAllocatorPage
{
public:
...
std::shared_ptr<bee::DescriptorAllocatorPage> DescriptorAllocator::CreateAllocatorPage()
{
...
DescriptorAllocation DescriptorAllocator::Allocate(uint32_t numDescriptors)
{
    ...
    while (iter != m_AvailableHeaps.end())
    {
        ...
        allocation = allocatorPage->AllocateDescriptors(numDescriptors);
    ...
    if (allocation.IsNull())
    {
        ...
        allocation = newPage->AllocateDescriptors(numDescriptors);
    }
