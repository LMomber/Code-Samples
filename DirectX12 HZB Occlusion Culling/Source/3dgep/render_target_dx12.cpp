...
void RenderTarget::AttachTexture(AttachmentPoint attachmentPoint, std::shared_ptr<DX12Texture> texture)
{
...
std::shared_ptr<DX12Texture> RenderTarget::GetTexture(AttachmentPoint attachmentPoint) const
{
...
void RenderTarget::Resize(uint32_t width, uint32_t height) { Resize(DirectX::XMUINT2(width, height)); }

DirectX::XMUINT2 RenderTarget::GetSize() const { return m_Size; }

uint32_t RenderTarget::GetWidth() const { return m_Size.x; }

uint32_t RenderTarget::GetHeight() const { return m_Size.y; }
...
const std::vector<std::shared_ptr<DX12Texture>>& RenderTarget::GetTextures() const { return m_Textures; }
