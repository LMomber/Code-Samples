...
DX12Texture::DX12Texture(Device& device, const D3D12_RESOURCE_DESC& resourceDesc, const D3D12_CLEAR_VALUE* clearValue)
    : DX12Resource(device, resourceDesc, clearValue)
{
    CreateViews();
}

DX12Texture::DX12Texture(Device& device, ComPtr<ID3D12Resource> resource, const D3D12_CLEAR_VALUE* clearValue)
    : DX12Resource(device, resource, clearValue)
{
    CreateViews();
}
...
void DX12Texture::Resize(uint32_t width, uint32_t height, uint16_t depthOrArraySize)
{
    if (m_d3d12Resource)
    {
...
void DX12Texture::CreateViews()
{
    if (m_d3d12Resource)
    {
        ...
        {
            if (!CheckDSVSupport() && desc.Format == DXGI_FORMAT_R32_TYPELESS)
            {
        ...
        {
            if (!CheckSRVSupport() && desc.Format == DXGI_FORMAT_R32_TYPELESS)
            {
...
D3D12_CPU_DESCRIPTOR_HANDLE DX12Texture::GetRenderTargetView() const { return m_RenderTargetView.GetDescriptorHandle(); }

D3D12_CPU_DESCRIPTOR_HANDLE DX12Texture::GetDepthStencilView() const { return m_DepthStencilView.GetDescriptorHandle(); }

D3D12_CPU_DESCRIPTOR_HANDLE DX12Texture::GetShaderResourceView() const { return m_ShaderResourceView.GetDescriptorHandle(); }

D3D12_CPU_DESCRIPTOR_HANDLE DX12Texture::GetUnorderedAccessView(uint32_t mip) const
{
    return m_UnorderedAccessView.GetDescriptorHandle(mip);
}

bool DX12Texture::HasAlpha() const
{
...
size_t DX12Texture::BitsPerPixel() const
{
...
DXGI_FORMAT DX12Texture::GetSRGBFormat(DXGI_FORMAT format)
{
...
void bee::DX12Texture::TransitionUAVToSRV(ID3D12GraphicsCommandList* commandList) 
{
    if (m_d3d12Resource)
    {
