...
DynamicDescriptorHeap::DynamicDescriptorHeap(Device& device,
...
void DynamicDescriptorHeap::ParseRootSignature(const std::shared_ptr<RootSignature>& rootSignature)
{
    ...
    assert(currentOffset <= m_NumDescriptorsPerHeap &&
           "The root signature requires more than the maximum number of descriptors per descriptor heap. Consider increasing "
           "the maximum number of descriptors per descriptor heap.");
}

void DynamicDescriptorHeap::StageDescriptors(uint32_t rootParameterIndex,
                                             uint32_t offset,
...
void DynamicDescriptorHeap::CommitStagedDescriptorsForDraw(CommandList& commandList)
{
    ...
    CommitInlineDescriptors(commandList,
                            m_InlineCBV,
                            m_StaleCBVBitMask,
